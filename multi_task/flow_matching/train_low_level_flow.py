...
#   python pianomime/multi_task/flow_matching/train_low_level_flow.py dataset_ll.zarr
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train PianoMime low-level policy with conditional flow matching.")
    parser.add_argument("dataset_path", type=str, help="Path to low-level zarr dataset, e.g. dataset_ll.zarr")
    ...
    parser.add_argument("--save-every", "--save_every", dest="save_every", type=int, default=100)
...
def default_ckpt_path(dataset_path: str, ckpt_dir: str, run_name: str | None) -> Path:
    run_name = run_name or f"FM-LL-{sanitize_name(dataset_path)}"
    return Path(ckpt_dir) / f"checkpoint_{run_name}.ckpt"
...
def main() -> None:
    ...
    print(f"[FM-LL train] dataset={args.dataset_path}")
    print(f"[FM-LL train] checkpoint will be saved to {ckpt_path}")
    print(f"[FM-LL train] time_sampler={fm_cfg.sampler_name}, time_scale={fm_cfg.time_scale}")
        ...
        for epoch_idx in tglobal:
            ...
            with tqdm(dataloader, desc="Batch", leave=False) as tepoch:
                for nbatch in tepoch:
                    ...
                    tepoch.set_postfix(loss=loss_value, t=float(logs["t_mean"].cpu()))

            tglobal.set_postfix(loss=float(np.mean(epoch_loss)), lr=lr_scheduler.get_last_lr()[0])
            if args.save_every > 0 and epoch_idx % args.save_every == 0:
                torch.save(ema.averaged_model.state_dict(), ckpt_path)
                print(f"Saved low-level FM checkpoint at epoch {epoch_idx}: {ckpt_path}")

    torch.save(ema.averaged_model.state_dict(), ckpt_path)
    print(f"Done. Final low-level FM checkpoint: {ckpt_path}")
